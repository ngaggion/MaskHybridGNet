...
class EncoderConv(nn.Module):
    def __init__(self, config):
        ...
        for i in range(len(self.filters)):
            ...
            self.dconv_down.append(residualBlock(in_channels, out_channels))
        
        # Final convolutional layer
        self.dconv_final = residualBlock(self.filters[-1], self.filters[-1])
...
class Hybrid(nn.Module):
    ...
    def raster(self, x, organs, resolution=512):
        output = []
        for organ in np.unique(organs):
            landmarks_organ = x[organs == organ].unsqueeze(0)
